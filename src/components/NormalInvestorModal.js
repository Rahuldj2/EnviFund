...
import { normalinvestorprojectcontext } from "./NormalInvestorCard";
import { useSession, signIn } from "next-auth/react";
const NormalProjectModal = () => {
    const { data: session } = useSession();
     // const project = useContext(projectInfoContext);
    const project=useContext(normalinvestorprojectcontext);
        ...
        <div className="p-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md">
                <div className="mb-4">
                    ...
                    <p>{project.project_details.project_title}</p>
                    {/* <p>{project.project_id}</p> */}
                </div>
                <div className="mb-4">
                    <h3 className="text-lg font-semibold">Location:</h3>
                    <p>{project.project_details.location}</p>
                </div>
                <div className="mb-4">
                    <h3 className="text-lg font-semibold">Funding Goal:</h3>
                    <p>{project.project_details.funding_goal} {project.isCryptoProject ? "MATIC" : "INR"}</p>
                </div>
                <div className="mb-4">
                    <h3 className="text-lg font-semibold">Current Balance:</h3>
                    <p>{project.project_details.funding_amount} {project.isCryptoProject ? "MATIC" : "INR"}</p>
                </div>
                <div className="mb-4">
                    <h3 className="text-lg font-semibold">Description:</h3>
                    <p>{project.project_details.description}</p>
                </div>
                <div className="mb-4">
                    <h3 className="text-lg font-semibold">Owner Email:</h3>
                    <p>{project.project_details.project_lead_id}</p>
                </div>
                <div className="mb-4">
                    <h3 className="text-lg font-semibold">Image URL:</h3>
                    <p>{project.project_details.imageURL}</p>
                </div>
